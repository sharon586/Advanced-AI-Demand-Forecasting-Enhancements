...
const modelColors = {
  'Random Forest': 'text-violet-400',
  'XGBoost': 'text-pink-400',
  'Linear Regression': 'text-blue-400',
  'ARIMA': 'text-green-400',
}

const modelBg = {
  'Random Forest': 'bg-violet-900/30 border-violet-700/40',
  'XGBoost': 'bg-pink-900/30 border-pink-700/40',
  'Linear Regression': 'bg-blue-900/30 border-blue-700/40',
  'ARIMA': 'bg-green-900/30 border-green-700/40',
}
  ...
  return (
    <Layout title="Forecast History">
      <div className="flex items-center justify-between mb-6">
        <p className="text-gray-400">All past forecast runs with accuracy metrics</p>
      ...
      <div className="flex gap-3 mb-6 p-4 bg-[#1B1538] rounded-2xl border border-[#312B56]">
        ...
        <select value={modelFilter} onChange={e => setModelFilter(e.target.value)}
          ...
          {['All', 'Random Forest', 'XGBoost', 'Linear Regression', 'ARIMA'].map(m => (
      ...
      <div className="bg-[#211A45] rounded-2xl p-6 border border-[#39306A]">
        <h2 className="text-lg font-bold mb-5">All Forecast Runs</h2>
              ...
              {filtered.map((h, i) => (
                <tr key={i} className="border-b border-[#312B56] hover:bg-[#2D2257] transition">
                  ...
                  <td className="py-3 px-3">
                    <span className={`px-2 py-1 rounded-lg text-xs font-semibold border ${modelBg[h.model]} ${modelColors[h.model]}`}>
                      {h.model}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    <span className="text-green-400 font-bold">{h.accuracy}</span>
                  </td>
